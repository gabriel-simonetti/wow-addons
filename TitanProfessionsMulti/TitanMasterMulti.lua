...
local function OnClick(self, button)
	if (button == "MiddleButton") and profs[2] ~= nil then
		local prof = profs[2][INFOS_INDEX.NAME]
		if profs[2][INFOS_INDEX.NAME] == L["mining"] then
			prof = L["miningskills"]
		elseif profs[2][INFOS_INDEX.NAME] == L["skinning"] then
			prof = ""
		elseif profs[2][INFOS_INDEX.NAME] == L["herbalism"] then
			prof = L["herbalismskills"]
		end
		CastSpellByName(prof)
	end
	if (button == "LeftButton") and profs[1] ~= nil then
		local prof = profs[1][INFOS_INDEX.NAME]
		if profs[1][INFOS_INDEX.NAME] == L["mining"] then
			prof = L["miningskills"]
		elseif profs[1][INFOS_INDEX.NAME] == L["skinning"] then
			prof = ""
		elseif profs[1][INFOS_INDEX.NAME] == L["herbalism"] then
			prof = L["herbalismskills"]
		end
		CastSpellByName(prof)
	end
